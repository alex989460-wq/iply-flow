...
export default function KnowledgeBaseDialog({ open, onOpenChange }: Props) {
  ...
  const seedStarter = async () => {
    ...
    const { data, error } = await (supabase.from('ai_knowledge_entries' as any) as any).insert(rows).select();
    setSaving(false);
    if (error) {
  ...
  const removeEntry = async (id: string) => {
    ...
    const { error } = await (supabase.from('ai_knowledge_entries' as any) as any).delete().eq('id', id);
    if (error) {
      toast({ title: 'Erro', description: error.message, variant: 'destructive' });
      return;
    }
    ...
    try {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        if (e.id.startsWith('new-')) {
          await (supabase.from('ai_knowledge_entries' as any) as any).insert(payload);
        } else {
          await (supabase.from('ai_knowledge_entries' as any) as any).update(payload).eq('id', e.id);
        }
      }
