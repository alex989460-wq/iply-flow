...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Processa em background para não estourar timeout do cliente.
    const runImport = async () => {
      ...
      const flush = async (phone: string, current: any[]) => {
        ...
        const { data: existing } = await supabase
          ...
          .select("id")
          .eq("user_id", userId).eq("source", "evolution").eq("contact_phone", phone)
          .eq("started_at", first.created_at).eq("ended_at", last.created_at)
          .maybeSingle();
        if (existing?.id) { skippedDuplicate++; return; }

        const { error: insertError } = await supabase.from("ai_training_conversations").insert({
          user_id: userId, source: "evolution", contact_phone: phone,
          ...
          message_count: current.length, status: "imported",
          raw: current.map(m => ({
      ...
      try {
        ...
        while (processed < (totalMessages ?? Infinity) && processed < limitMessages) {
          ...
          if (jobCheck?.status === "cancelled") {
            ...
            openContacts.clear();
            await supabase.from("ai_training_jobs").update({
              status: "cancelled", processed, finished_at: new Date().toISOString(),
        ...
        await supabase.from("ai_training_jobs").update({
          ...
          message: `Erro: ${String((e as Error).message)}`,
        }).eq("id", job!.id);
      }
    };

    // @ts-ignore - EdgeRuntime disponível no Deno Deploy
    if (typeof EdgeRuntime !== "undefined") EdgeRuntime.waitUntil(runImport());
    else runImport();

    return json({ ok: true, started: true, job_id: job!.id, total: totalMessages ?? 0 }, 202);
  } catch (e) {
