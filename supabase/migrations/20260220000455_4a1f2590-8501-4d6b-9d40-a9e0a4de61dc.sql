...
-- Update dashboard stats to include new customers this month
...
-- Update renew trigger to use calendar months instead of fixed days
...
BEGIN
  IF NEW.confirmed = true AND OLD.confirmed = false THEN
    ...
    IF plan_duration IS NOT NULL THEN
      ...
      SET due_date = CURRENT_DATE + (plan_duration || ' days')::interval,
          status = 'ativa'
